...
pub fn filter_dfg_absolute<'a>(dfg: &DirectlyFollowsGraph<'a>, threshold: u32) -> DirectlyFollowsGraph<'a> {
    ...
    let to_remove: Vec<_> = result.activities.keys().filter(|&a| {
        result.get_ingoing_df_relations(a).is_empty() &&
            result.get_outgoing_df_relations(a).is_empty() }).cloned().collect();

    to_remove.iter().for_each( |a| result.remove_activity(a));

    result
}

pub fn inverted_filter_dfg_absolute<'a>(dfg: &DirectlyFollowsGraph<'a>, threshold: u32) ->
